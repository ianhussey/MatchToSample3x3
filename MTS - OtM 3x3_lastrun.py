...
# -*- coding: utf-8 -*-
"""
This experiment was created using PsychoPy2 Experiment Builder (v1.82.01), Sun Apr 17 23:41:38 2016
...
for thisBlocks_loop in blocks_loop:
    ...
    thisPostblock_loop = postblock_loop.trialList[0]  # so we can initialise stimuli with some values
    # abbreviate parameter names if possible (e.g. rgb=thisPostblock_loop.rgb)
    if thisPostblock_loop != None:
        for paramName in thisPostblock_loop.keys():
            exec(paramName + '= thisPostblock_loop.' + paramName)
    
    for thisPostblock_loop in postblock_loop:
        currentLoop = postblock_loop
        # abbreviate parameter names if possible (e.g. rgb = thisPostblock_loop.rgb)
        if thisPostblock_loop != None:
            for paramName in thisPostblock_loop.keys():
                exec(paramName + '= thisPostblock_loop.' + paramName)
    ...
    # completed 1 repeats of 'postblock_loop'
